...
def train_eval():
    ...
    train_loader = DataLoader(trainSet,
                              batch_size=CFG.batch_size-len(CFG.kernels),
                              shuffle=True,
                              num_workers=5,
                              drop_last=False,
                              pin_memory=True)
    ...
    backbone_optimizer = torch.optim.Adam(model.textCNN.parameters(), lr=CFG.lr)
    ...
    for epoch in range(start_epoch+1, CFG.epochs):
        train(epoch, model, train_loader, backbone_optimizer, D_optimizer, G_optimizer, loss_function)
...
def train(epoch, model, train_loader, backbone_optimizer, D_optimizer, G_optimizer, loss_function):
    model.train()
    with torch.enable_grad():
        ...
        for batch in tqdm(train_loader):
            ...
            model.textCNN.requires_grad_(True)
